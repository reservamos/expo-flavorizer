...
## Plugin System and Configuration Flexibility

Expo Flavorizer is designed to be flexible and project-agnostic. It allows teams to define their own project structure while maintaining the benefits of Expo's plugin system.

### How It Works

```mermaid
graph TD
    A[flavorizer.config.json] -->|Defines| B[Flavor Configuration]
    B -->|Maps to| C[Project Structure]
    C -->|Updates| D[app.config.js]
    C -->|Generates| E[Assets]
    C -->|Configures| F[Native Projects]

    subgraph "Project Configuration"
        G[flavorizer-plugin.config.js] -->|Defines| C
    end

    subgraph "Expo Flavorizer"
        B
        C
    end
...
Teams can define how their flavor configuration maps to their project structure by creating a `flavorizer-plugin.config.js` file:

```javascript
// flavorizer-plugin.config.js
module.exports = {
  // Map flavorizer config to project structure
  mapConfig: (flavorConfig) => ({
    // Map to app.config.js structure
    appConfig: {
      // Your project-specific mapping
    },
    // Map to asset locations
    assets: {
...
This approach:
* Keeps your project structure private
* Allows flexible configuration mapping
* Maintains compatibility with Expo's plugin system
* Supports different project organizations

### Example Usage

```json
// flavorizer.config.json
{
  "flavors": {
    "apple": {
      "appName": "Apple App",
